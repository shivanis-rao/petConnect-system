...
const getAdopterPrefillData = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: 'Prefill data fetched successfully', data: user });
  } catch (error) {
...
const submitAdoptionApplication = async (req, res) => {
  ...
  try {
    ...
    const {
      shelterId, currentOccupation, address,
      livingArrangement, familyAgreement,
      landlordAllowsPets, petCareWhenAway,
    } = req.body;

    if (!shelterId || !currentOccupation || !address || !livingArrangement || !landlordAllowsPets || !petCareWhenAway)
      return res.status(400).json({ message: 'All fields are required' });

    const validLivingArrangements = ['Family', 'I live alone', 'House/Room mates'];
    ...
    if (!validLivingArrangements.includes(livingArrangement))
      return res.status(400).json({ message: 'Invalid livingArrangement value' });
    ...
    if (!validLandlord.includes(landlordAllowsPets))
      return res.status(400).json({ message: 'Invalid landlordAllowsPets value' });
...
try {
  ...
  return res.status(500).json({ message: 'findOne failed', detail: findErr.message });
    ...
    const application = await AdoptionApplication.create({
      userId, petId, shelterId,
      first_name:         user.first_name,
      last_name:          user.last_name,
      phoneNumber:        user.phone,
      email:              user.email,
      petExperienceYears: user.pet_experience_years || 0,
      currentOccupation, address, livingArrangement,
...
const getMyApplications = async (req, res) => {
  try {
    const applications = await AdoptionApplication.findAll({
      ...
      include: [
        { model: Pet,     as: 'pet',     attributes: ['id', 'name', 'species', 'breed', 'age'] },
...
const getApplicationsForShelter = async (req, res) => {
  try {
    ...
    const applications = await AdoptionApplication.findAll({
      ...
      include: [
        { model: Pet,  as: 'pet',       attributes: ['id', 'name', 'species', 'breed', 'age'] },
        { model: User, as: 'applicant', attributes: ['id', 'first_name', 'last_name', 'email', 'phone'] },
      ],
      order: [['createdAt', 'DESC']],
    });
    return res.status(200).json({ message: 'Shelter applications fetched successfully', data: applications });
  } catch (error) {
...
const updateApplicationStatus = async (req, res) => {
  try {
    ...
    return res.status(200).json({ message: `Application status updated to '${status}'`, data: application });
  } catch (error) {
